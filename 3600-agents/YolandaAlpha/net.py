...
================================
Input : 13 feature planes × 8×8 board  →  832 floats
          (rat-belief channel removed — the net learns purely from board state)
Trunk : Dense(832→256)→ReLU → Dense(256→256)→ReLU → Dense(256→128)→ReLU
Value : Dense(128→64)→ReLU → Dense(64→1)→tanh   →  scalar ∈ (−1, +1)
Policy: Dense(128→64)→ReLU → Dense(64→100)       →  logits over move vocabulary

Move vocabulary (100 slots)
-----------------------------
  0 –  3 : PLAIN  UP / RIGHT / DOWN / LEFT
  4 –  7 : PRIME  UP / RIGHT / DOWN / LEFT
  8 – 35 : CARPET  index = 8 + dir*7 + (roll-1),  dir ∈ 0–3, roll ∈ 1–7
 36 – 99 : SEARCH  index = 36 + y*8 + x

Feature channels (13 × 64, cell index = y*8+x)
-------------------------------------------------
  0  player worker position  (1-hot)
  1  opponent worker position (1-hot)
  2  PRIMED cells
  3  CARPET cells
  4  BLOCKED cells
  5  SPACE cells
  6  player points / 100   (scalar broadcast over all 64 cells)
  7  opponent points / 100
  8  player turns_left / 40
  9  opponent turns_left / 40
 10  player time_left / 360
 11  opponent time_left / 360
 12  is_player_a  (1.0 if player_worker.is_player_a else 0.0)

Weights are persisted as a .npz file of flattened JAX leaves so the file
is readable by numpy alone on the tournament machine.
"""
...
# optax is imported lazily inside training functions — not needed for inference
...
def board_to_features(board) -> np.ndarray:
    ...
    def _i(loc): return loc[1] * BOARD_SIZE + loc[0]

    feat[0, _i(pw.get_location())] = 1.0
    feat[1, _i(ow.get_location())] = 1.0
    ...
    feat[6]  = float(pw.get_points())  / 100.0
    feat[7]  = float(ow.get_points())  / 100.0
    feat[8]  = float(pw.turns_left)    / 40.0
    feat[9]  = float(ow.turns_left)    / 40.0
    feat[10] = float(pw.time_left)     / 360.0
    feat[11] = float(ow.time_left)     / 360.0
    feat[12] = 1.0 if pw.is_player_a else 0.0
...
class DualNet(nn.Module):
    ...
    def __call__(self, x) -> Tuple:
        ...
        for dim in self.trunk_dims:
            h = nn.Dense(dim,
                         kernel_init=nn.initializers.he_uniform())(h)
            h = nn.relu(h)

        # Value head
        v = nn.Dense(self.value_dim,
                     kernel_init=nn.initializers.he_uniform())(h)
        v = nn.relu(v)
        v = nn.Dense(1,
                     kernel_init=nn.initializers.glorot_uniform())(v)
        v = nn.tanh(v)
        v = v[:, 0]          # (B,)

        # Policy head
        p = nn.Dense(self.policy_dim,
                     kernel_init=nn.initializers.he_uniform())(h)
        p = nn.relu(p)
        p = nn.Dense(self.n_moves,
                     kernel_init=nn.initializers.glorot_uniform())(p)
...
def make_optimizer(lr: float = 3e-4):
    """Adam with gradient clipping. Requires optax (training only)."""
    import optax
    return optax.chain(
...
# None by default — initialised by set_optimizer() in train.py before training.
_TX = None
...
def set_optimizer(lr: float):
    ...
    Call this in train.py before the first train_step().
    Requires optax — training only, not needed for tournament inference.
    """
    global _TX, train_step
    _TX = make_optimizer(lr)   # make_optimizer does lazy optax import
