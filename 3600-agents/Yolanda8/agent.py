...
# Search budget
_TIME_BUFFER = 0.10     # was 0.25; keep a small safety margin
_MAX_DEPTH   = 20       # was 8; let iterative deepening run until time expires
_MIN_SECS    = 0.01     # absolute floor; adaptive check is the real guard
_BRANCH_EST  = 4.0      # estimated effective branching factor for adaptive timing
...
class Negamax:
    ...
    def best_move(self, board: Board, budget: float) -> Move:
        ...
        # Pre-predict belief snapshots up to _MAX_DEPTH plies
        ...
        best = moves[0]
        prev_val = 0.0
        last_iter_time = 0.0     # time the last completed depth took

        for depth in range(1, _MAX_DEPTH + 1):
            remaining = self._dead - time.perf_counter()

            # ── Adaptive depth check ─────────────────────────────────
            # Don't start a new depth if we estimate it will timeout.
            # At depth > 1 we have timing data from the previous depth;
            # the next depth's cost is roughly _BRANCH_EST × the last.
            # At depth 1–2 we always proceed (they're cheap).
            if depth > 2 and last_iter_time > 0:
                estimated_cost = last_iter_time * _BRANCH_EST
                if estimated_cost > remaining:
                    break
            elif remaining < _MIN_SECS:
                break
...
class PlayerAgent:
    ...
    def _decide(self, board: Board, time_left: Callable) -> Move:
        ...
        # The budget cap is deliberately high — iterative deepening
        # with adaptive timing (in best_move) handles early termination
        # when the search completes before the budget runs out.
        if turns_left > 25:
            time_frac = 1.25     # early: slightly more for strategic setup
        elif turns_left > 12:
            time_frac = 1.0      # mid: baseline
        else:
            time_frac = 0.85     # late: slightly less, simpler positions
        per_turn = t_remaining / max(1, turns_left) * time_frac
        budget   = max(0.1, min(15.0, per_turn * 0.95))
