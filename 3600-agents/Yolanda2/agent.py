...
class PlayerAgent:
    ...
    def __init__(self, board_state, transition_matrix=None, time_left: Callable = None):
        ...
        self._pending_search_loc = None
        self._correct_searches = 0
    ...
    def commentate(self) -> str:
        ...
        return (
            ...
            f"Correct rat guesses: {self._correct_searches}/{self._total_searches}"
        )

    # ------------------------------------------------------------------
    def _record_search_result(self, search_loc: Tuple[int, int], search_hit: bool) -> None:
        if search_loc is None:
            return
        if self._pending_search_loc != search_loc and (search_loc, search_hit) == self._prev_player_search:
            return
        if self.belief is not None:
            self.belief.update_search(search_loc, search_hit)
        self._prev_player_search = (search_loc, search_hit)
        self._pending_search_loc = None
        if search_hit:
            self._correct_searches += 1
    ...
    def play(self, board_state: Board, sensor_data: Tuple, time_left: Callable) -> Move:
        ...
        if self.belief is not None:
            ...
            if my_loc is not None and (
                self._pending_search_loc == my_loc
                or board_state.player_search != self._prev_player_search
            ):
                self._record_search_result(my_loc, my_hit)
    ...
    def _decide(self, board_state: Board, time_left: Callable) -> Move:
        ...
        if t_left <= 0:
            ...
            moves = board_state.get_valid_moves(exclude_search=True)
            move = moves[0] if moves else Move.search((0, 0))
            self._pending_search_loc = move.search_loc if move.move_type == MoveType.SEARCH else None
        ...
        if self.belief is not None:
            ...
            if search_ev >= threshold:
                self._pending_search_loc = search_loc
                self._total_searches += 1
        ...
        self._pending_search_loc = best_move.search_loc if best_move.move_type == MoveType.SEARCH else None
        if best_move.move_type == MoveType.SEARCH:
            self._total_searches += 1
        return best_move
